...
class Router(object):
    ...
    def doRoute(self, start, end, preserve_route=False):
        """Do the routing"""
        self.searchEnd = end
        closed = [start]
        self.queue = []
        ...
        try:
            for i, weight in list(self.data.routing[start].items()):
        ...
        while count < 1000000:
            ...
            if x == end:
                ...
                if preserve_route:
                    return ('success', routeNodes, nextItem)
                else:
                    return ('success', routeNodes)
            closed.append(x)
            try:
                for i, weight in list(self.data.routing[x].items()):
                    if not i in closed:
